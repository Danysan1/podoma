    ...
    if (mode == "init"){
        ...
        if (CONFIG.hasOwnProperty("OVERPASS_URL") && CONFIG.OVERPASS_URL != null && (csvMembers != null || live_enabled)){
            ...
            if [[ "\$missing_features" != "||" ]]; then
                ...
                curl -d @${CONFIG.WORK_DIR}/missing_osm.overpass --retry 3 --retry-delay 5 --retry-max-time 250 -f -o "${CONFIG.WORK_DIR}/missing_osm.xml" -A "Podoma/1.0 (${CONFIG.WEBSITE_URL})" -X POST ${CONFIG.OVERPASS_URL} || echo "ERROR: Overpass API request failed"
