...
- **The one exception is `pdm_mapper_counts.amount`**, the cumulative distinct-mapper count, whose anchor is applied at write time via the `:project_start_date` psql var ([30_projects_update.js](db/30_projects_update.js) → [33_projects_contribs.sql](db/33_projects_contribs.sql)). `COUNT(DISTINCT userid)` is not subtractable, so — unlike the feature counts, which [stats.pug](website/templates/components/stats.pug) narrows at read time by differencing the two endpoints of the hard-anchored series — a series anchored at `start_date` can never yield the soft-window figure. Two things follow: `amount` is `0` on every date before the soft start (while `amount_1d`/`amount_30d` stay real, so the usual "cumulative ≥ rolling" invariant breaks there, visible in the `/projects/:slug/mappers` API), and toggling a project's `use_soft_dates` or editing a soft start only re-anchors dates processed from then on — earlier rows keep the old anchor until a reprocess.

### Configuration

`config.json` is the file actually loaded at runtime and is **gitignored** — it is a local copy of `config.italia.json` (the tracked instance config: Italy PBF/OSH sources, Italian map center/zoom, GeoJSON bounds, Matomo Tag Manager, `OVERPASS_FATAL`). Edit `config.italia.json` for anything that should persist, then re-copy. Every config key is documented in the "General configuration" section of [docs/DEVELOP.md](docs/DEVELOP.md). `use_soft_dates` is a per-project `info.json` setting (top-level, alongside `soft_start_date`/`soft_end_date`), not an instance config key — see below.
...
- **`use_soft_dates` is a per-project `info.json` key and is invisible to SQL.** It reaches the database through `20_changes_update.js`, which writes `soft_start_date`/`soft_end_date` into `pdm_projects` *only* when that project's setting is enabled; SQL then simply reads `COALESCE(soft_start_date, start_date)`. Consequence: toggling the setting, or editing soft dates in an `info.json`, only takes effect at the next `changes:update` run.
